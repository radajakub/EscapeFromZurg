...
dfs(left, Left, Right, [left_to_right(N1, N2)|Sol], Capacity) :-
    pair(T1, T2, Left),
    toyname(T1, N1),
    toyname(T2, N2),
    ...
    NewCapacity >= 0,
    dfs(right, NewLeft, [T1,T2|Right], Sol, NewCapacity).

dfs(right, [], _Right, [], _).
dfs(right, Left, Right, [right_to_left(N)|Sol], Capacity) :-
    member(T, Right),
    toyname(T, N),
    ...
    NewCapacity is Capacity - Cost,
    NewCapacity >= 0,
    dfs(left, [T|Left], NewRight, Sol, NewCapacity).
